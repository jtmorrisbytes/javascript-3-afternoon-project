...
//Code Here
let evenNumbers = mixedNumbers.filter(numbers => numbers % 2 == 0)// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map(element => element * 1.07) // = prices.map(/* Provide Your Callback Here );
...
//Code Here
let totalPopulation = populations.reduce((accumulator,element) => accumulator + element) //  = populations.reduce(/* Provide Your Callback Here */)
...
let orderTotals = orders.reduce(
  (accumulator,element)=> 
  accumulator + (element.price * (1.0 + element.tax) )
  )// Code here
...
*/
throw new Error(" come back to solve problem six")
let bobsTotal = 
  purchases.filter(
    (purchase) => purchase.owner === "Barry"
  )
  
  console.log("bobsTotalFilter",bobsTotal)
  bobsTotal = bobsTotal.reduce(
    (accumulator,purchase) =>
    accumulator += purchase.price
  );
